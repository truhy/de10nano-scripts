...
The altera_de10nano.cfg is an optional script, which I don't use, but it would go into the board scripts folder, for example:
C:\devtools\xpack-openocd-0.12.0-2\share\openocd\scripts\board

The USB Blaster II is the official programming JTAG adapter for Intel FPGAs, which is already on the DE10-Nano board. It requires a driver and the firmware file (blaster_6810.hex) to work. The driver and firmware file comes bundled with "Quartus Prime Programmer and Tools" and also "Quartus Prime Lite" so you can install either one. If you're not going to use "Quartus Prime Lite" you may prefer the smaller size of "Quartus Prime Programmer and Tools". During the setup make sure to include the driver as part of the installation.

OpenOCD requires the path to the blaster_6810.hex file to be specified.  Since the installation of Quartus creates the environment variable QUARTUS_ROOTDIR or QSYS_ROOTDIR, I've added TCL code to the target script (altera-usb-blaster2.cfg) that will use these to find the path to the blaster_6810.hex file.
